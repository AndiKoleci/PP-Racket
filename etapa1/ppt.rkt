...
(define (multiply M V) (multiply-tail M V null))
(define (multiply-tail M V acc)
  (if (null? M)
      acc
      (multiply-tail (cdr M) V (reverse (cons (dot-product (car M) V) (reverse acc))))
  )
)
...
; Utilizați recursivitate pe coadă.
(define (apply-matrix-transformations Ts ppt) (apply-matrix-transformations-tail Ts ppt))
(define (apply-matrix-transformations-tail Ts acc)
  (cond
    ((null? Ts) acc)
    ((= (car Ts) 1) (apply-matrix-transformations-tail (cdr Ts) (multiply T1 acc)))
    ((= (car Ts) 2) (apply-matrix-transformations-tail (cdr Ts) (multiply T2 acc)))
    ((= (car Ts) 3) (apply-matrix-transformations-tail (cdr Ts) (multiply T3 acc)))
    (else -1)
  )
)
...
(define (get-nth-ppt-from-matrix-transformations n)
  (apply-matrix-transformations (get-transformations n) (cons 3 (cons 4 (cons 5 null))))
)
